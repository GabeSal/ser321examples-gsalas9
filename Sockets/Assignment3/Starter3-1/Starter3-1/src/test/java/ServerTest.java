...
public class ServerTest {
    ...
    @Test
    public void tempConvertValid() throws IOException {
        JSONObject req = new JSONObject();
        ...
        assertEquals(32.0, res.getDouble("result"), 0.1);
    }

    @Test
    public void tempConvertMissingField() throws IOException {
        JSONObject req = new JSONObject();
        req.put("type", "tempconvert");
        req.put("value", 100.0);
        // missing fromUnit and toUnit
        ...
        assertFalse(res.getBoolean("ok"));
        assertTrue(res.getString("message").contains("Field"));
    }

    @Test
    public void dadJokeAdd() throws IOException {
        JSONObject req = new JSONObject();
        req.put("type", "dadjoke");
        req.put("action", "add");
        req.put("joke", "Why did the coffee file a police report? It got mugged!");
        ...
        assertTrue(res.getBoolean("ok"));
        assertTrue(res.getString("message").contains("Joke added"));
    }

    @Test
    public void dadJokeGet() throws IOException {
        JSONObject req = new JSONObject();
        req.put("type", "dadjoke");
        req.put("action", "get");
        ...
        assertTrue(res.getBoolean("ok"));
        assertTrue(res.has("jokeID"));
        assertTrue(res.has("joke"));
        assertTrue(res.has("rating"));
    }

    @Test
    public void dadJokeRateInvalid() throws IOException {
        JSONObject req = new JSONObject();
        req.put("type", "dadjoke");
        req.put("action", "rate");
        req.put("jokeID", 1);
        req.put("rating", "6"); // invalid rating
        ...
        assertFalse(res.getBoolean("ok"));
        assertEquals("Rating is out of bounds", res.getString("message"));
    }
}