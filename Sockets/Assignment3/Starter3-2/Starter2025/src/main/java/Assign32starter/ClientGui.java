...
public class ClientGui implements Assign32starter.OutputPanel.EventHandlers {
	...
	public ClientGui(String host, int port) throws IOException {
		...
		// you can move the open and closing of a connection to totally differnt places if you like
	...
	public void submitClicked() {
		...
		// just very simple if the input text is riddle send the riddle type, if not then send start
		// this of course needs to be changed it is only for you to see how you can send different requests
		...
		try {
			os.writeObject(message.toString()); // this will crash the server, since it is not a JSON and thus the server will not handle it.
		...
		try {
			...
			JSONObject reply = new JSONObject(this.bufferedReader.readLine());
			System.out.println(reply);
			// this just sets the whole response into the UI -- should of course only print the needed information in your game
